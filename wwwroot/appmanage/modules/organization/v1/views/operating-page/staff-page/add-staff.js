define([
    ...
    return {
        ...
        methods:{
            ...
            addStaffSave:function(){
                var self = this;
                this.filterListPositionID();

                return new Promise(function (resolve, reject) {
                    Employee.AddEmployee(self.addStaffFormData)
                        .then(function () {
                            resolve();
                            self.$message({
                                message: '新增员工成功！',
            ...
            PositionByID:function(data){
           
                var uid = data.operationData.data.ID,
                    that = this;
                Employee.GetPositionsByID({ID:uid}).then(function(res){
                    if(res.PostionID){
                        that.ListPositionList.push(res);
                    }
                });
            },
        //   用户头像上传
        ...
        created :function(){
            if(this.incomingData.nativeEdit){
                this.PositionByID(this.incomingData);
            }
        },
