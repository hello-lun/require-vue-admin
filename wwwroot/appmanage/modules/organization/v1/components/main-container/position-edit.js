define([
    ...
    return {
        ...
        created: function () {
            this.asyncGetPositionData();
        },

        methods: {
            asyncGetPositionData: function () {
                var self = this;
                Department.getByID({ 
                    ID: self.incomingData.id 
                }).then(function (data) {
                    self.formData.Name = data.name;
                    self.formData.IsAllAuthority = data.isAllAuthority;
                    self.formData.ParentName = data.parentName;
                    if (data.parentName == null) {
                        self.formData.ParentName = "商户节点";
                    }
                    self.formData.ParentID = data.parentID;
                    self.formData.ID = self.incomingData.id;
                    self.formData.Type = 3;
                    self.tableData = data.listUser;
                    self.userlist = data.listUser;
                });
            ...
            handleDelClick(row) {
                ...
                this.userlist = newNodeData;

                if (this.$refs.resetuserlist != null && this.$refs.resetuserlist != undefined) {

                    this.$refs.resetuserlist.setCheckedNodes(newNodeData);
                }
            ...
            submit: function (close) {
                ...
                Department.update(self.formData)
                    .then(function (d) {
                        if (d != null) {
                            if (d.isSuccess == true) {
                                self.$message({
        ...
        <side-bar-form
            ...
            <el-form-item label="包含员工:" prop="name">
                ...
                <el-dialog 
                    ...
                    :close-on-click-modal="false" 
                    :close-on-press-escape="false">
                    <el-aside class="dialogtree" width="630px">
                        <userlist-tree   
                            ref="resetuserlist"  
                            :selectUser="userlist"> 
                        </userlist-tree>
