define([
  ...
  var assign = Object.assign ? Object.assign : _.extend || (() => {});
  ...
  return function (options) {
    ...
    service = {
      ...
      addFile: function (file, callback) {
        if (file) {
          var fileKey = CryptoJSMD5(file.name + ':' + file.size).toString();
          var fileObj;
      ...
      upload: function (param, callback) {
        ...
        if (!param.trunkSize) { param.trunkSize = 65 * 1024; }
        if (!service.uploadFile) {
          opts.onError({
            errMsg: '未选择需上传的文件',
          });
